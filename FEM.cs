...
public class FEM<T> 
{
    ...
    private void global() {
        ...
        for (int index_fin_el = 0; index_fin_el < Elems.Length; index_fin_el++) {
            ...
            (Matrix loc_mat, Vector local_f) = local(index_fin_el);
    ...
    private void EntryMatInGlobalMatrix(Matrix mat, int[] index) {
    ...
    private (Matrix, Vector) local(int index_fin_el) {
        // Подсчет компонент
        ...
        Vector local_f      = build_F(index_fin_el, hx, hy);    // Построение локальной правой части
        Matrix M            = build_M(index_fin_el, hx, hy);    // Построение матрицы массы (M)
        Matrix G            = build_G(index_fin_el, hx, hy);    // Построение матрицы жесткости (G)
        Matrix local_matrix = G + M;                            // Локальная матрицы (G + M)

        return (local_matrix, local_f);
    }

    //* Построение вектора правой части
    private Vector build_F(int index_fin_el, double hx, double hy) {
    ...
    private Matrix build_M(int index_fin_el, double hx, double hy) {
        // Подсчет коэффициента
        double coef = (Function.gamma * hx * hy) / 36.0;
    ...
    private void WriteMatrix() {
        ...
        File.WriteAllText(Path + "/matrix/ig.txt", String.Join(" ", slau.ig));
        File.WriteAllText(Path + "/matrix/jg.txt", String.Join(" ", slau.jg));
